...
hvor $Q(\cdot)$ er kvantilfunktionen for $\chi^2$-fordelingen. I stedet for en faktor på
$$k=\frac{2}{T}.$$
...
Processen køres kun for regressionerne for aktivafkastene på det fulde datasæt, for at have et kvantitativt bud på deres optimale prædiktionsvariable. Eftersom fokus er på porteføljeallokering, restringeres datasættet herefter til kun at indeholde variablene, der forbliver i modellen efter *Top-down* processen. Dernæst holdes variablene op i mod de univariate og multiple regressioner samt korrelationsanalysen, for at undgå mulige multikollinaritetsproblemer. Skulle der være problemer med multikollinarietet fjernes evt. problematiske variable. Hvis de forklarende variable herefter er forskellige på tværs af aktivklasserne foretages en kvalitativ vurdering, som har til formål at udvælge en delmængde af de tilbageværende variable, som fuldt kommer til at indgå i VAR(1)-modellen. 

Slutteligt foretages en *Variance Inflation Factor* analyse, som et sidste tjek for multikollinaritet. Analysen foretages i tre skridt. For samtlige foreklarende variable laves følgende regression
$$x_t^i=\alpha+\bm{\beta}^j + \bm{x}_t^{j}+\varepsilon_t.$$

Dernæst beregnes *VIF*-værdien for hvert $\beta^j$ som
$$\text{VIF}_i=\frac{1}{1-R_i^2},$$

hvor $R_i^2$ stammer fra regressionen i ovenstående Ligning ??. Det sidste skridt består i at undersøge størrelsesordenen af multikollinaritet, ved at tjekke størrelsen på $\text{VIF}_i$. En generel regel er, at multikollinariteten er høj når $\text{VIF}_i>10$. Kvadratroden af $\text{VIF}$ indikerer, hvor meget standardfejlen øges, sammenlignet med tilfældet, hvor variablen hypotetisk ingen korrelation har til de andre variable i modellen. 
...
\begin{align*}
r_{t+1}^{\text{rf}}&=\phi_0^{\text{rf}} + \phi_1^{\text{rf}} x_{t}^{\text{dp}} +  \phi_2^{\text{rf}} x_{t}^{\text{bm}} + \phi_3^{\text{rf}} x_{t}^{\text{avar}} + \phi_4^{\text{rf}} x_{t}^{\text{smb}} + \phi_5^{\text{rf}} x_{t}^{\text{tb}} + \phi_6^{\text{rf}} x_{t}^{\text{ys}} + \phi_7^{\text{rf}} x_{t}^{\text{cs}} + \phi_8^{\text{rf}} x_{t}^{\text{ds}} + \phi_9^{\text{rf}} x_{t}^{\text{fr}}\\
rx_{t+1}^{\text{a}}&=\phi_0^{\text{a}} + \phi_1^{\text{a}} rx_{t}^{\text{a}} +  \phi_2^{\text{a}} x_{t}^{\text{bm}} + \phi_3^{\text{a}} x_{t}^{\text{avar}} + \phi_4^{\text{a}} x_{t}^{\text{smb}} + \phi_5^{\text{a}} x_{t}^{\text{tb}} + \phi_6^{\text{a}} x_{t}^{\text{ts}} + \phi_7^{\text{a}} x_{t}^{\text{ys}} + \phi_8^{\text{a}} x_{t}^{\text{cs}} + \phi_9^{\text{a}} x_{t}^{\text{fr}}\\
rx_{t+1}^{\text{s}}&=\phi_0^{\text{s}} + \phi_1^{\text{s}} rx_{t}^{\text{s}} +  \phi_2^{\text{s}} x_{t}^{\text{avar}} + \phi_3^{\text{s}} x_{t}^{\text{smb}} + \phi_4^{\text{s}} x_{t}^{\text{tb}} + \phi_5^{\text{s}} x_{t}^{\text{ys}} + \phi_6^{\text{s}} x_{t}^{\text{cs}} + \phi_7^{\text{s}} x_{t}^{\text{fr}}\\
rx_{t+1}^{\text{v}}&=\phi_0^{\text{v}}+ \phi_1^{\text{v}} rx_{t}^{\text{v}} +  \phi_2^{\text{v}} x_{t}^{\text{avar}} + \phi_3^{\text{v}} x_{t}^{\text{smb}} + \phi_4^{\text{v}} x_{t}^{\text{tb}} + \phi_5^{\text{v}} x_{t}^{\text{ys}} + \phi_6^{\text{v}} x_{t}^{\text{cs}} + \phi_7^{\text{v}} x_{t}^{\text{fr}}.
\end{align*}

Dermed haves et temporalt system, som herfra vil udtyndes kvalitativt. 

Givet den høje korrelation mellem rentespændende samt *Federal Funds Rate*, foretages en suboptimal beslutning ved at fjerne samtlige rentespænd på nær *Yield Spread*. Det korte rentespænd beholdes eftersom denne variabel generelt har udvist størst signifikans på tværs af alle aktivklasser i både den univariate samt multiple regressionsanalyse.
...
colnames(VAR_TABLE) <- c("Name", "$r_t^{\\text{rf}}$", "$rx_t^{\\text{a}}$", "$rx_t^{\\text{s}}$", "$rx_t^{\\text{v}}$", '$x_t^{\\text{bm}}$', '$x_t^{\\text{avar}}$', '$x_t^{\\text{smb}}$', '$x_t^{\\text{b}}$', '$x_t^{\\text{ys}}$', "Afskæring")

rownames(VAR_TABLE) <- c(1:27)
