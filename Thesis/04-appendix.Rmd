...
## Momenter af den lognormale fordeling

Det præsenterede porteføljevalg i den empiriske analyse tog udgangspunkt i en investor med potens nytte. Dette udmyntede sig i antagelsen om lognormale afkast. Lad en stokastisk variabel $Y=\exp(X)\sim \mathcal{LN}(\mu,\sigma^2)$ have den egenskab, at $X=\log(Y)\sim\mathcal{N}(\mu,\sigma^2)$.
...
\begin{equation}
\frac{R_{p,t+1}}{R_{0,t+1}} = 1 + \alpha_t (\frac{R_{t+1}}{R_{0,t+1}}-1), (\#eq:logl2)
\end{equation}
...
\begin{equation}
r_{p,t+1} - r_{0,t+1} = \log (1+\alpha_t (\exp(r_{t+1}-r_{0,t+1})-1)). (\#eq:logl3)
\end{equation}
