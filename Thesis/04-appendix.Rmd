...
Ovenstående Ligning ?? viser den ikke-lineære relatioen mellem log afkastet og log udbyttet, altså er det andet led på højreside ikke-lineært, $\log(1+\exp(d_{t+1}-p_{t+1}))$. Ved brug af en første ordens Taylor udvidelse omkring middelværdien, $\overline{d_t-p_t}$, kan denne relation approksimeres. 

Her bliver den første afledte, $f'(\overline{d_t-p_t})$ fundet ved brug af kædereglen til
$$f'(\overline{d_{t+1}-p_{t+1}})=\frac{1}{1+\exp(\overline{d_{t+1}-p_{t+1}})}\exp(\overline{d_{t+1}-p_{t+1}}).$$

Dermed bliver første ordens Taylor udvidelsen
$$f(x)=\log(1+\exp(\overline{d_t-p_t}))+\frac{1}{1+\exp(\overline{d_t-p_t})}\exp(\overline{d_t-p_t})((d_t-p_t)- \overline{d_t-p_t})$$
