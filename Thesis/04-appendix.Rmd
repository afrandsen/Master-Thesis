...
hvor $k$ er notationen for kvadratisk. Fra ovenstående kan det ses, at *ARA* for kvadratisk nytte er stigende i $W_{t+1}$. Hvilket ikke er retsmæssigt, ift. den generelle holdning blandt eksisterende litteratur. 
...
### Eksponentiel nytte
Eksponentiel nytte betegnes undertiden *Constant Absolute Risk Aversion*. Eksponentiel nytte antager at variablene i modellen er normalfordelte. Lad $\theta\in\mathbb{R}$ være parametren i modellen, da vil eksponentiel nytte være defineret ved
\begin{equation*}
...
### Potens nytte
Potens nytte kræver, at afkast af aktiverne og formue er lognormalfordelte. Potens nytte er bedre kendt under betegnelsen *Constant Relative Risk Aversion*. Lad parameteren $\gamma\in\mathbb{R}$ være givet, da er nyttefunktionen defineret for $W_{t+1}\geq 0$, som
\begin{align*}
...
I denne Sektion udledes specialtilfældet af Ligning \@ref(eq:Potnytte), dvs. nyttefunktions form for risikoaversion $\gamma$ lig én. Ved brug af *l'Hôpital's regel*. Reglen er defineret for to funktioner, $f(\cdot)$ og $g(\cdot)$, som er differentiable på et åbent interval $I$, undtagen for et fast punkt $c\in I$. Hvis
...
\begin{equation}
1+R_{t+1}=\frac{P_{t+1}+D_{t+1}}{P_t}. (\#eq:fundaafk)
\end{equation}

Ved at tage logaritmefunktionen på Ligning \@ref(eq:fundaafk) kan følgende udledning opnås
\begin{align*}
...
\begin{equation}
r_{t+1}=p_{t+1} +\log\left(1+\exp(d_{t+1}-p_{t+1})\right)-p_t. (\#eq:logafknl)
\end{equation}

Ovenstående Ligning \@ref(eq:logafknl) viser den ikke-lineære relation mellem log afkastet og log udbyttet, altså er det andet led på højreside ikke-lineært, $\log\left(1+\exp(d_{t+1}-p_{t+1})\right)$. Ved brug af en første ordens Taylor udvidelse omkring middelværdien, $\overline{d_t-p_t}$, kan denne relation approksimeres. Herfra sættes $x=\log\left(1+\exp(d_{t+1}-p_{t+1})\right)$.
...
&=\left(1-\rho\right)\left(d_{t+1}-p_{t+1}\right)+c. (\#eq:tayaktafk)
\end{align}

Indsættes Ligning \@ref(eq:tayaktafk) nu ind i Ligning \@ref(eq:logafknl) opnås den lineære relation for én-periode log aktieafkastet.
\begin{align}
...
&\approx \rho p_{t+1} + \left(1-\rho\right) d_{t+1} - p_t + c. (\#eq:tildynggm)
\end{align}

### Dynamic Gordon Growth Model

Identiteten fra *Dynamic Gordon Growth Model* findes fra Ligning \@ref(eq:tildynggm), og ved indsættelse af $d_t-d_t$
\begin{align*}
...
\begin{equation}
d_t-p_t=\rho (d_{t+1}-p_{t+1})+r_{t+1}-\Delta d_{t+1}-c. (\#eq:deltadtpt)
\end{equation}

Tages nu den betingede forventning af Ligning \@ref(eq:deltadtpt) på tidspunkt $t$, og ved samtidig rekursion fås
\begin{align*}
...
\begin{equation}
P_t^n=\frac{1}{\left(1+Y_t^n\right)^n}. (\#eq:fundaobl)
\end{equation}
...
Benyttes Ligning \@ref(eq:fundaobl) følger det nu, at én-periode bruto afkastet på obligationen fra $t$ fra $t+1$ bliver
\begin{align*}
...
&=\frac{1}{n}\sum_{i=0}^{n-1} r_{t+i+1}^{n-i}. (\#eq:ytn)
\end{align}
...
For at udlede rentespændet fratrækkes $y_t^1$ på begge sider af Ligning \@ref(eq:ytn), så
\begin{align*}
...
r_{t+1}^{c,n}&=\log\left(1+\exp\left(c- p_{t+1}^{c,n}\right)\right) + p_{t+1}^{c,n} - p_t^{c,n}.(\#eq:kuponliglog)
\end{align}

I ovenstående Ligning \@ref(eq:kuponliglog), er $\log\left(1+\exp\left(c- p_{t+1}^{c,n}\right)\right)$ ikke-lineær. For at løse dette anvendes en første ordens Taylor approksimation omkring middelværdien, $\overline{c- p_{t+1}^{c,n}}$. Herfra sættes $x=\log\left(1+\exp\left(c- p_{t+1}^{c,n}\right)\right)$. 
...
Dette appendiks vil udlede den optimale porteføljestrategi, som beskrevet i Sektion \@ref(rekloes). Udledningen vil følge [@JurVic2011] Appendiks C, som beskriver metoden, med enkelte dele som varierer.
...
\begin{equation}
\max_{\bm{\alpha}_{t+k-1}^1} \mathbb{E}_{t+k-1}\left[\exp((1-\gamma)r_{p,t+k})\right] = \exp\left((1-\gamma) \mathbb{E}_{t+k-1}\left[r_{p,t+k}\right]+\frac{1}{2}(1-\gamma)^2 \mathbb{V}_{t+k-1}\left[r_{p,t+k}\right]\right).(\#eq:polmaks)
\end{equation}
...
Den følgende førsteordensbetingelse, fremkommer efter substituion af ovenstående momenter ind i Maksimeringsproblemet \@ref(eq:polmaks) og dernæst første ordens differentiation 
...
\begin{equation}
\mathbb{E}_{t+k-2}\left[ \exp\left( \left(1-\gamma\right) \left(\left(B_1^1 + \begin{pmatrix} 1 & \bm{\alpha}_{t+k-2}^{2} & \bm{0}\end{pmatrix} \right) \bm{z}_{t+k-1} + \bm{z}_{t+k-1}'B_2^1 \bm{z}_{t+k-1}  \right)\right) \right]. (\#eq:normfordeltstok)
\end{equation}

Resultatet af denne beregning vil herefter substitueres ind i den oprindelige optimeringsfunktion for at opnå forvetningen af *value*-funktionen betinget på investorens porteføljevalg, der herefter bliver maksimeret over $\bm{\alpha}_{t+k-2}^{2}$.
...
Dette betyder også at Ligning \@ref(eq:normfordeltstok) beregner den forventede værdi af det eksponentielle kvadratiske polynomium i en normalfordelt stokastisk variabel.
...
Kombineres ovenstående hjælpevariable med Ligning \@ref(eq:normfordeltstok) og den betingende fordeling af $\bm{z}_{t+k-1}$ fås
\begin{align}
\mathbb{E}_{t+k-2}\Big[ \exp\Big( C_1^{2\prime} \bm{z}_{t+k-1} + \bm{z}_{t+k-1}' C_2^2 \bm{z}_{t+k-1} \Big) ] = &\mathbb{E}_{t+k-2}[ \exp \Big( C_1^{2\prime} \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} + \bm{v}_{t+k-2} \Big) \notag\\
                                                                                                  ...
                                                                                                  &+ \bm{v}_{t+k-2}\Big) \Big) \Big]. (\#eq:midtk2)
\end{align}

Ovenstående Ligning \@ref(eq:midtk2) har led i eksponentialet, som enten er konstanter, lineære kombinationer af $\bm{v}_{t+k-2}$ stødene eller kvadratiske i $\bm{v}_{t+k-2}$. Derfor kan vi omarrangere leddene som følger
\begin{align}
\mathbb{E}_{t+k-2}\Big[\exp\Big(\cdot\Big)\Big] &= \exp\Big( C_1^{2\prime} \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big) + \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big)' C_2^2 \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big)\Big) \notag\\
                                ...
                                &=\exp\Big( C_1^{2\prime} \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big) + \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big)' C_2^2 \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big)\Big)\notag\\
                                &\,\,\quad \mathbb{E}_{t+k-2}\Big[ \exp\Big(\bm{v}_{t+k-2}' C_2^2 \bm{v}_{t+k-2} \Big) \exp\Big( C_1^{2\prime} + 2\Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2}\Big)' \Tilde{C}_2^{2} \Big)   \bm{v}_{t+k-2}\Big], (\#eq:langlorteformel)
\end{align}
...
Dette medfører at Ligning \@ref(eq:langlorteformel) simplificeres til
\begin{align}
\lim_{\Delta t \to 0}\mathbb{E}_{t+k-2}\Big[\exp\Big(\cdot\Big)\Big] &=\exp\Big( C_1^{2\prime} \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big) + \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2} \Big)' C_2^2 \Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2}\Big) +  \text{tr}\Big(C_2^2 \bm{\Sigma}_v\Big)\Big)\notag\\
                                &\,\,\quad \mathbb{E}_{t+k-2}\Big[ \exp\Big( C_1^{2\prime} + 2\Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2}\Big)' \Tilde{C}_2^{2} \Big)   \bm{v}_{t+k-2}\Big], (\#eq:llf2)
\end{align}

her observeres, at forventningen i Ligning \@ref(eq:llf2) er forventningen af en lognormal stokastisk variabel, som er givet ved
\begin{align}
...
&=\exp\Big(\frac{1}{2} \Big(C_1^{2\prime} + 2\Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2}\Big)' \Tilde{C}_2^{2}\Big)' \bm{\Sigma}_v \Big(C_1^{2\prime} + 2\Big(\bm{\Phi}_0+\bm{\Phi}_1\bm{z}_{t+k-2}\Big)' \Tilde{C}_2^{2}\Big)\Big) (\#eq:llf3)
\end{align}

Kombineres Ligning \@ref(eq:llf2) og Ligning \@ref(eq:llf3) ses, at Ligning \@ref(eq:normfordeltstok) i grænsen for kontinuert tid er lig
\begin{align}
\lim_{\Delta t \to 0} &\mathbb{E}_{t+k-2}\Big[ \exp\Big( C_1^{2\prime} \bm{z}_{t+k-1} + \bm{z}_{t+k-1}' C_2^2 \bm{z}_{t+k-1} \Big)\Big]\notag\\
                      &=\exp\Big( \text{tr}\Big(C_2^2\bm{\Sigma}_v\Big) + D_0^{\Big(t+k-2\Big)} + \frac{1}{2} D_1^{\Big(t+k-2\Big)} \bm{\Sigma}_v^{-1} D_1^{\Big(t+k-2\Big)'} \Big),(\#eq:komapproks)
\end{align}
...
&= \frac{\left| \bm{\Sigma}_v \right|^{-\tfrac{1}{2}}}{\left| \left(\bm{\Sigma}_v^{-1} - 2 D_2^{\left(t+k-2\right)}\right) \right|^{\tfrac{1}{2}}}\exp\left(D_0^{\left(t+k-2\right)} + \frac{1}{2} D_1^{\left(t+k-2\right)} \left(\bm{\Omega}^1\right)^{-1} D_1^{\left(t+k-2\right)\prime}\right), (\#eq:llf5)
\end{align}
...
For et udlede den optimale portefølje strategi, droppes konstanten fra Ligning \@ref(eq:llf5), siden dette ikke påvirker optimum. Altså ser investorens problem på tidspunkt $(t+k-2)$ således ud
...
hvor
\begin{align}
A_0^2&=\left(\bm{\Sigma}_{xx} - \left(1-\gamma\right)\Tilde{\bm{\Omega}}_{xx}^1\right)^{-1} \left(\bm{\Phi}_0^x + \frac{1}{2} \bm{\sigma}_x^2 + \left(1-\gamma\right) \left(\Tilde{\bm{\Omega}}_{x0}^1 + \Tilde{\bm{\Omega}}_{x}^1 \left(B_1^{1\prime} + 2\Tilde{B}_2^{1}\bm{\Phi}_0\right)\right)\right) (\#eq:a0fra1)\\
A_1^2&=\left(\bm{\Sigma}_{xx} - \left(1-\gamma\right)\Tilde{\bm{\Omega}}_{xx}^1\right)^{-1}\left(\bm{\Phi}_1^x+2\left(1-\gamma\right) \Tilde{\bm{\Omega}}_{x}^1 \Tilde{B} _2^{1}\bm{\Phi}_1\right). (\#eq:a1fra1)
\end{align}

På samme vis kan beregninger baseret på Ligning \@ref(eq:komapproks), give følgende resultat
\begin{align}
A_0^2&=\frac{1}{\gamma} \bm{\Sigma}_{xx}^{-1} \left(\bm{\Phi}_0^x \frac{1}{2} \bm{\sigma}_x^2 + \left(1-\gamma \right) \left(\bm{\Sigma}_{x0} + \bm{\Sigma}_x \left(B_1^{1\prime} + 2\Tilde{B}_2^{1}\bm{\Phi}_0 \right)\right)\right)(\#eq:a0fra2)\\
A_1^2&=\frac{1}{\gamma} \bm{\Sigma}_{xx}^{-1} \left(\bm{\Phi}_1^x + 2\left(1-\gamma\right) \bm{\Sigma}_x \Tilde{B}_2^{1}\bm{\Phi}_1\right).(\#eq:a1fra2)
\end{align}

Ligning \@ref(eq:a0fra1) og Ligning \@ref(eq:a1fra1) er identiske med Ligning \@ref(eq:a0fra2) og Ligning \@ref(eq:a1fra2), bortset fra at $\Tilde{\bm{\Omega}}^1$ er udskiftet med $\bm{\Sigma}_v$.
